...
describe('broken', function () {
    ...
    it('should request a DOI without special data', function (done) {
        //resetDatabase();
        ...
        authToken = dataLogin.data.authToken;
        ...
        setTimeout(
            ...
            //check if rawtransaction arrived on bob
            ...
            const resultRawTransaction = getHttpPOST(node_url_bob, realdata);
            logBlockchain('result:',resultRawTransaction.toString().indexOf(txid)>0);

                // should check if a server connects to pop3', function(done){
                ...
                // should confirm the link of the doi-request-email on bobs dapp', function(done){
                setTimeout(
                    Meteor.bindEnvironment(function () {
                        ...
                        setTimeout(
                            Meteor.bindEnvironment(function () {
                                ...
                                // should check if alice local db has information about a confirmed doi.', function(done){
                                ...
                                const dataNameShow = {"jsonrpc": "1.0", "id":"name_show", "method": "name_show", "params": [nameId] };
                                const realdataNameShow = { auth: auth, data: dataNameShow, headers: headers };
                                const resultNameShow = getHttpPOST(node_url_bob, realdataNameShow);
                                logBlockchain('resultGenerate:',result);
                                const statusNameShow = resultNameShow.statusCode;
                                chai.assert.equal(200, statusNameShow);
                                chai.expect(statusNameShow.data.error).to.be.null;
                                chai.expect(statusNameShow.data.result).to.not.be.null;
                                chai.assert.equal(nameId, resultNameShow.data.result.name);
                                ...
                                const urlVerify = dapp_url_alice+'/api/v1/opt-in/verify';
                                ...
                                logBlockchain('verifying opt-in:', dataVerify);
                                const realdataVerify = { data: dataVerify, headers: headersVerify };
                                const resultVerify = getHttpGETdata(urlVerify, realdataVerify);

                                logBlockchain('result /opt-in/verify:', resultVerify);
                                const statusVerify = result.statusCode;
