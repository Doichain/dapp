...
USER root
RUN  apt-get update && \
  apt-get install --no-install-recommends -y apt-utils python python-dev python-distribute python-pip xvfb wget curl \
  xdg-utils libxtst6 libxss1 libxrender1 libxrandr2 libxi6 libxcursor1 libxcomposite1  libpango-1.0-0 libpangocairo-1.0-0 \
