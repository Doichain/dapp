node {
   ...
   // try {
        docker.image("mongo:3.2").withRun("-p 27018:27017"){
            docker.image("sameersbn/bind:latest").withRun("-it --dns=127.0.0.1 ---name=mail --hostname=mail --name=bind --publish=53:53/udp --volume=/bind:/data --env='ROOT_PASSWORD=generated-password'") { b ->
            def BIND_IP = sh(script: "sudo docker inspect bind | jq '.[0].NetworkSettings.IPAddress'", returnStdout: true).trim()
                ...
                docker.image("esminis/mail-server-postfix-vm-pop3d").withRun("-it --dns=${BIND_IP} --name=mail -p 8443:8443 -p 25:25 -p 465:465 -p 995:995"){
